...
const transform: AxiosTransform = {
  ...
  transformRequestHook: (res, options) => {
    if (res && res.data) {
      ...
      if (retCode === '990101' || retCode === '990110') {
      ...
      } else if (retCode === '990103') {
        ElMessage({
          message: '系统正在维护，请稍后再试',
          type: 'warning',
        });
        // } else if (retCode === '990104') {
        //   ElMessage({
        //     message: '服务器内部错误',
        //     type: 'warning',
        //   });
  ...
  responseInterceptorsCatch: (error: any) => {
    ...
    return backoff.then((configs) => request.request(configs));
  },
};
